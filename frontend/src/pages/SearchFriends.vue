    ...
    export default
    {
        ...
        methods: 
        {
            ...
            async sendInvitation(friendName)
            {
                try 
                {
                    await axios({method: 'post', url: `http://localhost:5000/friends/${this.id}/${friendName}`, headers: {'Authorization': 'Bearer ' + localStorage.getItem('token')} });
            ...
            async searchFriends(userName)
            {
                try 
                {
                    ...
                    const response = await axios.get(`http://localhost:5000/user/${userName}/${this.id}`, { headers: {Authorization: 'Bearer ' + localStorage.getItem('token')} });
                    var logins = response.data;
                    try 
                    {
                        logins.forEach( async element => {
                            const response2 = await axios.get(`http://localhost:5000/friendstatus/${this.id}/${element.Login}`, { headers: {Authorization: 'Bearer ' + localStorage.getItem('token')} });
